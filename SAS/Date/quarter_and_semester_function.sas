...
proc fcmp outlib=work.cat_function.test ;
	function Quarter(DateDay,Language $) $ ;
		...
		if Lg='EN' then do;
			select (qd);
				when (1) q=compress('Q1');
				when (2) q=compress('Q2');
				when (3) q=compress('Q3');
				when (4) q=compress('Q4');
				otherwise put 'Not a date !';
			end;
		end;
		else do;
			select (qd);
				when (1) q=compress('T1');
				when (2) q=compress('T2');
				when (3) q=compress('T3');
				when (4) q=compress('T4');
				otherwise put 'Not a date !';
			end;
		end;
	return(q);
